...
public class USSDServiceKT extends AccessibilityService {
    ...
    public void onAccessibilityEvent(AccessibilityEvent event) {
        ...
        Log.d("onAccessibilityEvent", String.format(
        ...
        if (LoginView(event) && notInputText(event)) {
            ...
            Log.d("isUSSDWidget","catch a USSD widget/Window");
            if (notInputText(event)) {
                // not more input panels / LAST MESSAGE
                // sent 'OK' button
                Log.d("notInputText","No inputText found & closing USSD process");
                clickOnButton(event, 0);
                ussd.stopRunning();
                ussd.callbackInvoke.over(response);
            } else {
    ...
    public static void cancel() {
        clickOnButton(event, 0);
        Log.d("cancel","Trying to close/cancel USSD process by clicked in first button ");
    }
